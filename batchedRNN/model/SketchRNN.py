...
class SketchRNNDecoder(nn.Module):
    def __init__(self):
        ...
        # unidirectional lstm:
        self.numGMMParams = (args.x_dim * 2 + 1) * args.num_mixtures # mu, sigma
        self.lstm = nn.LSTM(args.z_dim + 2 * args.x_dim, args.decoder_h_dim, args.n_layers, dropout=args.decoder_layer_dropout)
        self.fc_params = nn.Linear(args.decoder_h_dim, self.numGMMParams) #6*M: mu_x, mu_y, rho, sigma_x, sigma_y for GMM and pi for weight of mixture in GMM
    ...
    def forward(self, inputs, z, hidden_cell=None):
        ...
        print(mixture_mu.size(), mixture_sigma.size(), pi.size())
        assert False, "finish implementation"
        pi = F.softmax(pi.t().squeeze()).view(args.sequence_len, -1, args.num_mixtures)
...
class SketchRNN(nn.Module):
    ...
    def forward(self, batch):
        ...
        embedded = batch.contiguous().view(args.sequence_len, args.batch_size, args.x_dim * args.channels)
        z, mu, sigma = self.encoder(embedded)
        if args.cuda:
            startOfSequence = Variable(torch.stack([torch.zeros(self.decoder.numGMMParams)]\
             * args.batch_size).cuda()).unsqueeze(0)
        else:
            startOfSequence = Variable(torch.stack([torch.zeros(self.decoder.numGMMParams)] * args.batch_size)).unsqueeze(0)
        batch_init = torch.cat([startOfSequence, embedded], 0)
        z_stack = torch.cat([startOfSequence], (args.sequence_len + 1))
        inputs = torch.cat([batch_init, z_stack], 2)
        pi, mixture_mu, mixture_sigma = self.decoder(inputs, z)
